        ...
        try:
            if radar_configs['data_type'] == 'RI' or radar_configs['data_type'] == 'AP':  # drop this format
                ...
                if isinstance(chirp_id, int):
                    ...
                    radar_npy_win_RD = np.zeros((self.win_size, ramap_rsize, 128, 2), dtype=np.float32)
                    radar_npy_win_AD = np.zeros((self.win_size, ramap_asize, 128, 2), dtype=np.float32)
                    for idx, frameid in enumerate(
                            range(data_id, data_id + self.win_size * self.step, self.step)):
                        for cid, c in enumerate(chirp_id):
                            ...
                            if self.norm_type == "real&image":
                                if self.print_num <= 0:
                                    print("NormType is real&image", self.use_filter)
                                    self.print_num += 1
                                radar_npy_win[idx, cid, :, :, 0] = (radar_npy_win[idx, cid, :, :, 0] - self.mean_std_list[f'RA{str(self.use_filter)}_real_mean']) / self.mean_std_list[f'RA{str(self.use_filter)}_real_std']
                                radar_npy_win[idx, cid, :, :, 1] = (radar_npy_win[idx, cid, :, :, 1] - self.mean_std_list[f'RA{str(self.use_filter)}_imag_mean']) / self.mean_std_list[f'RA{str(self.use_filter)}_imag_std']
                            elif self.norm_type == "none":
                                if self.print_num <= 0:
                                    print("NormType is none", self.use_filter)
                                    self.print_num += 1
                        # get RD
                        rd_npy_path = radar_paths[frameid][0]
                        rd_npy_path = rd_npy_path.replace("raw_frame_RA", "raw_frame_RD")
                        rd_npy_path = rd_npy_path.replace(rd_npy_path.split('/')[-1], f"{int(rd_npy_path.split('_')[-2].split('/')[-1]):09d}.npy")
                        radar_npy_win_RD[idx, ...] = np.load(rd_npy_path)

                        # get AD
                        ad_npy_path = radar_paths[frameid][0]
                        ad_npy_path = ad_npy_path.replace("raw_frame_RA", "raw_frame_AD")
                        ad_npy_path = ad_npy_path.replace(ad_npy_path.split('/')[-1], f"{int(ad_npy_path.split('_')[-2].split('/')[-1]):09d}.npy")
                        radar_npy_win_AD[idx, ...] = np.load(ad_npy_path)

                        radar_npy_win_RD[..., 0] = (radar_npy_win_RD[..., 0] - self.mean_std_list[f'RD{str(self.use_filter)}_real_mean']) / self.mean_std_list[f'RD{str(self.use_filter)}_real_std']
                        radar_npy_win_RD[..., 1] = (radar_npy_win_RD[..., 1] - self.mean_std_list[f'RD{str(self.use_filter)}_imag_mean']) / self.mean_std_list[f'RD{str(self.use_filter)}_imag_std']

                        radar_npy_win_AD[..., 0] = (radar_npy_win_AD[..., 0] - self.mean_std_list[f'AD{str(self.use_filter)}_real_mean']) / self.mean_std_list[f'AD{str(self.use_filter)}_real_std']
                        radar_npy_win_AD[..., 1] = (radar_npy_win_AD[..., 1] - self.mean_std_list[f'AD{str(self.use_filter)}_imag_mean']) / self.mean_std_list[f'AD{str(self.use_filter)}_imag_std']
