...
class CRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            if radar_configs['data_type'] == 'RI' or radar_configs['data_type'] == 'AP':  # drop this format
                ...
                if isinstance(chirp_id, int):
                    ...
                    img_npy_win = np.zeros((self.win_size, 480, 640, 3), dtype=np.float32)
                    for idx, frameid in enumerate(
                            range(data_id, data_id + self.win_size * self.step, self.step)):
                        for cid, c in enumerate(chirp_id):
                            ...
                            if self.norm_type == "real&image":
                                ...
                                if self.print_num <= 0:
                                    print("NormType is none", self.use_filter)
                                    self.print_num += 1
                        temp_img = cv2.cvtColor(cv2.imread(image_paths[frameid]), cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
                        temp_img[..., 0] = (temp_img[..., 0] - 0.485) / 0.229
                        temp_img[..., 1] = (temp_img[..., 1] - 0.456) / 0.224
                        temp_img[..., 2] = (temp_img[..., 2] - 0.406) / 0.225
                        temp_img = cv2.resize(temp_img, (640, 480))
                        img_npy_win[idx, ...] = temp_img
                        data_dict['image_paths'].append(image_paths[frameid])
                    crop_range_length = self.config_dict['radar_config']['ramap_rsize'] // self.rangeDownSample
        ...
        img_npy_win = np.transpose(img_npy_win, (3, 0, 1, 2))
        data_dict['img_data'] = img_npy_win
        ...
        if self.use_training:
            return data_dict['radar_data'], data_dict['img_data'], data_dict['anno']['confmaps'], data_dict['image_paths']
        return data_dict
