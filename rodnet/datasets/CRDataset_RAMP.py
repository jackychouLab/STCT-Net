            ...
            if radar_configs['data_type'] == 'ROD2021':
                if isinstance(chirp_id, int):
                    ...
                    if self.norm_type == "real&image":
                        ...
                        radar_npy_win_RD[0::2, :, :, 0] = (radar_npy_win_RD[0::2, :, :, 0] - self.mean_std_list[f'RD{str(self.use_filter)}_real_mean']) / self.mean_std_list[f'RD{str(self.use_filter)}_real_std']
                        radar_npy_win_RD[1::2, :, :, 0] = (radar_npy_win_RD[1::2, :, :, 0] - self.mean_std_list[f'RD{str(self.use_filter)}_imag_mean']) / self.mean_std_list[f'RD{str(self.use_filter)}_imag_std']

                        radar_npy_win_AD[0::2, :, :, 0] = (radar_npy_win_AD[0::2, :, :, 0] - self.mean_std_list[f'AD{str(self.use_filter)}_real_mean']) / self.mean_std_list[f'AD{str(self.use_filter)}_real_std']
                        radar_npy_win_AD[1::2, :, :, 0] = (radar_npy_win_AD[1::2, :, :, 0] - self.mean_std_list[f'AD{str(self.use_filter)}_imag_mean']) / self.mean_std_list[f'AD{str(self.use_filter)}_imag_std']
                    if self.print_num <= 0:
