...
class CRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            if radar_configs['data_type'] == 'ROD2021':
                if isinstance(chirp_id, int):
                    ...
                    img_npy_win = np.zeros((self.win_size, 300, 500, 3), dtype=np.float32)
                    for idx, frameid in enumerate(range(data_id, data_id + self.win_size * self.step, self.step)):
                        ...
                        temp_img = cv2.resize(temp_img, (500, 300))
        ...
        if self.use_training:
            return data_dict['radar_data'], data_dict['anno']['confmaps'], data_dict['image_paths'], data_dict['img_data']
        return data_dict