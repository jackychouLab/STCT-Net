...
def window_partition(x, window_size):
    ...
    Args:
        x: (B, D, H, W, C)
        window_size (tuple[int]): window sizes
...
class SwinTransformerSys3D(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if len(x.shape) == 6:
            # x_complex = torch.sum(x[:, 0, ...] + x[:, 1, ...] * 1j, dim=2, keepdim=True)
            # x = torch.cat((torch.real(x_complex), torch.imag(x_complex)), dim=2).transpose(1, 2).contiguous()
