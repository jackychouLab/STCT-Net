...
from .rcrosnet_exo import build_position_encoding, build_transformer
...
def window_partition2(x, window_size):
    ...
    if ratio > 1:
        windows = x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(-1, int(reduce(mul, window_size) / 4), C)
    else:
        windows = x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(-1, int(reduce(mul, window_size) * ratio), C)
    return windows
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, image_to_ra, mask=None, prev_v=None, prev_k=None, prev_q=None, is_decoder=False):
        """ Forward function.
        Args:
        ...
        if image_to_ra is not None:
            qkv2 = self.qkv(image_to_ra).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q = torch.add(q, qkv2[0])
...
class WindowAttention3D2(nn.Module):
    """ Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    Args:
        dim (int): Number of input channels.
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, image_to_ra, mask_matrix, prev_v, prev_k, prev_q, is_decoder):
        B, D, H, W, C = x.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)

        x = self.norm1(x)
        if image_to_ra is not None:
            image_to_ra = self.norm1(image_to_ra)
        ...
        if image_to_ra is not None:
            image_to_ra = window_partition(image_to_ra, window_size)
        # W-MSA/SW-MSA
        attn_windows, cross_attn_windows, v, k, q = self.attn(x_windows, image_to_ra, mask=attn_mask, prev_v=prev_v, prev_k=prev_k, prev_q=prev_q, is_decoder=is_decoder)  # B*nW, Wd*Wh*Ww, C
    ...
    def forward(self, x, image_to_ra, mask_matrix, prev_v, prev_k, prev_q, is_decoder=False):
        """ Forward function.
        ...
        if self.use_checkpoint:
            ...
            x, x2, v, k, q = self.forward_part1(x, image_to_ra, mask_matrix, prev_v, prev_k, prev_q, is_decoder)
...
class SwinTransformerBlock3D2(nn.Module):
    """ Swin Transformer Block.
    ...
    def forward_part1(self, x, mask_matrix, prev_v, prev_k, prev_q, is_decoder):
        ...
        x_windows = window_partition(shifted_x, window_size)  # B*nW, Wd*Wh*Ww, C
        # W-MSA/SW-MSA
        attn_windows, cross_attn_windows, v, k, q = self.attn(x_windows, mask=attn_mask, prev_v=prev_v,
                                                              prev_k=prev_k,
                                                              prev_q=prev_q,
                                                              is_decoder=is_decoder)  # B*nW, Wd*Wh*Ww, C
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, last=False):
        ...
        self.gn3a = nn.GroupNorm(num_groups=64, num_channels=256)
        self.gn3b = nn.GroupNorm(num_groups=64, num_channels=256)
...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.convt = nn.ConvTranspose3d(in_channels=dim, out_channels=int(dim / 2), kernel_size=(1, 2, 2),stride=(1, 2, 2))
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size=(7, 7, 7),
        ...
        self.convT = nn.ConvTranspose3d(in_channels=dim, out_channels=int(dim / 2), kernel_size=(2, 2, 2), stride=(2, 2, 2))
        ...
        self.convT2 = nn.ConvTranspose3d(in_channels=dim, out_channels=int(dim), kernel_size=(2, 2, 2),stride=(2, 2, 2))
        self.gn2 = nn.GroupNorm(num_groups=int(dim / 4), num_channels=int(dim))
        self.relu = nn.ReLU()
        self.prelu = nn.PReLU()
...
class BasicLayer_Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = nn.Conv3d(in_channels=int(dim), out_channels=int(dim), kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))

        self.gn = nn.GroupNorm(num_groups=int(dim / 4), num_channels=int(dim))

        self.relu = nn.ReLU()

    def forward(self, image, block_num):
        B, C, D, H, W = image.shape  # 1 32 8 72 120
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
        image = rearrange(image, 'b c d h w -> b d h w c')
        Dp = int(np.ceil(D / window_size[0])) * window_size[0]
        Hp = int(np.ceil(H / window_size[1])) * window_size[1]
        Wp = int(np.ceil(W / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, image.device)

        for idx, blk in enumerate(self.blocks):
            image, _, _, _ = blk(image, attn_mask, None, None, None, None)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv3d(in_channels=int(dim), out_channels=int(dim), kernel_size=(3, 3, 3),stride=(1, 1, 1), padding=(1, 1, 1))
        self.gn1 = nn.GroupNorm(num_groups=int(dim / 4), num_channels=int(dim))

        self.relu = nn.ReLU()

    def forward(self, ra, image_to_ra, block_num):

        ra = self.relu(self.gn1(self.conv1(ra)))# 1 32 2 64 64

        # ra
        B, C, D, H, W = ra.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
        ra = rearrange(ra, 'b c d h w -> b d h w c')
        image_to_ra = rearrange(image_to_ra, 'b c d h w -> b d h w c')

        Dp = int(np.ceil(D / window_size[0])) * window_size[0]
        Hp = int(np.ceil(H / window_size[1])) * window_size[1]
        Wp = int(np.ceil(W / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, ra.device)
        ...
        for idx, blk in enumerate(self.blocks):
            if idx % 2 == 0:
                if block_num == 0:
                    ra, v1, k1, q1 = blk(ra, image_to_ra, attn_mask, None, None, None)
                else:
                    ra, v1, k1, q1 = blk(ra, None, attn_mask, None, None, None)
            else:
                if block_num == 0:
                    ra, v2, k2, q2 = blk(ra, image_to_ra, attn_mask, None, None, None)
                else:
                    ra, v2, k2, q2 = blk(ra, None, attn_mask, None, None, None)

        if self.downsample is not None:
            x_ra = ra
            ra = ra.permute(0, 4, 1, 2, 3)
            x_ra = self.norm(x_ra)
            x_ra = x_ra.permute(0, 4, 1, 2, 3)
            if C != 128:
                ra = self.downsample(ra)
            else:
                ra = self.downsamplelast(ra)
        else:
            ra = self.norm(ra)
            ra = ra.permute(0, 4, 1, 2, 3)
            x_ra = ra

        return ra, x_ra, v1, k1, q1, v2, k2, q2
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=(2, 64, 64), patch_size=(4, 4, 4), in_chans=1, embed_dim=96, norm_layer=None):
        super().__init__()
        ...
        self.conv1a = nn.Conv3d(in_channels=self.in_chans, out_channels=int(embed_dim), kernel_size=(9, 5, 5),stride=(1, 1, 1), padding=(4, 2, 2))
        self.conv1b = nn.Conv3d(in_channels=int(embed_dim), out_channels=int(embed_dim), kernel_size=(9, 5, 5),stride=(2, 2, 2), padding=(4, 2, 2))

        self.gn1a = nn.GroupNorm(num_groups=int(embed_dim / 4), num_channels=int(embed_dim))
        self.gn1b = nn.GroupNorm(num_groups=int(embed_dim / 4), num_channels=int(embed_dim))

        self.conv2a = nn.Conv3d(in_channels=3, out_channels=int(embed_dim), kernel_size=(9, 5, 5), stride=(2, 1, 1), padding=(4, 2, 2))
        # self.conv2b = nn.Conv3d(in_channels=int(embed_dim), out_channels=int(embed_dim), kernel_size=(9, 5, 5), stride=(2, 2, 2), padding=(4, 2, 2))

        self.gn2a = nn.GroupNorm(num_groups=int(embed_dim / 4), num_channels=int(embed_dim))
        # self.gn2b = nn.GroupNorm(num_groups=int(embed_dim / 4), num_channels=int(embed_dim))


    def forward(self, radar, image_to_ra):

        radar = self.relu(self.gn1a(self.conv1a(radar)))  #
        radar = self.relu(self.gn1b(self.conv1b(radar)))  # 1 64 8 64 64

        image_to_ra = self.relu(self.gn2a(self.conv2a(image_to_ra)))  #
        # image_to_ra = self.relu(self.gn2b(self.conv2b(image_to_ra)))

        return radar, image_to_ra
...
class GenerateRangeAngle(nn.Module):
    ...
    def __init__(self, img_size=(2, 64, 64), patch_size=(4, 4, 4), in_chans=1, embed_dim=96, norm_layer=None):
        ...
        self.input_proj = nn.Conv2d(512, query_dim, kernel_size=1)
    ...
    def forward(self, image):
        ...
        for i in range(d):
            data = self.resnet18(image[:, :, i])
            data = data.reshape(b, 512, 10, 16)
            main_branch = self.input_proj(data)
            ...
            if i == 0:
                ...
                tmp_params = tmp[-1].unsqueeze(1)
            else:
                tmp = self.para_embed(hs)
                tmp[..., :self.query_dim] += reference
                tmp_params = torch.cat((tmp_params, tmp[-1].unsqueeze(1)), dim=1)# 2 16 6 128
        #process
        range_params = tmp_params[:, :, :3, :].unsqueeze(4)
        angle_params = tmp_params[:, :, 3:, :].unsqueeze(3)

        generate_ra = (range_params * angle_params).permute(0, 2, 1, 3, 4)
...
class SwinTransformerSys3D(nn.Module):
    def __init__(self, pretrained=None,
        ...
        self.patch_embed = PatchEmbed3D(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                        embed_dim=embed_dim,
                                        norm_layer=norm_layer if self.patch_norm else None)
        self.generate_rangeangle = GenerateRangeAngle(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                        embed_dim=embed_dim,
                                        norm_layer=norm_layer if self.patch_norm else None)
    ...
    def forward_features(self, radar, image_to_ra):
        ra, image_to_ra = self.patch_embed(radar, image_to_ra)  # 1 64 2 64 64

        # ra = self.pos_drop(ra)
        ra_downsample = []

        for i, layer in enumerate(self.layers):
            ra, x_ra, v1, k1, q1, v2, k2, q2 = layer(ra, image_to_ra, i)
            ra_downsample.append(x_ra)

        return ra_downsample

    def forward_up_features(self, decoder):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(decoder[2])
            else:
                x = torch.cat([x, decoder[2 - inx]], 1)
                B, C, D, H, W = x.shape
                x = x.flatten(2).transpose(1, 2)
    ...
    def forward(self, image, radar):
        image_to_ra = self.generate_rangeangle(image)
        ra_downsample = self.forward_features(radar, image_to_ra)
        x = self.forward_up_features(ra_downsample)
        result = self.Final(x)
        return result
...
class RC_ROSNet(nn.Module):
    def __init__(self, num_classes=3, zero_head=False, embed_dim=96, win_size=7, batchsize=4, device=None):
        super(RC_ROSNet, self).__init__()
        self.num_classes = num_classes
        ...
        self.swin_unet = SwinTransformerSys3D(img_size=(2, 64, 64),
                                              patch_size=(2, 2, 2),
                                              batchsize=16,
                                              in_chans=2,
    ...
    def forward(self, image, radar):
        logits = self.swin_unet(image, radar)
