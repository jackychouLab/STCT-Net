...
from rodnet.datasets.CRDataset_UAVRadar import CRDataset
...
from cruw.eval import evaluate_rodnet_seq_UAVRadar
import shutil
...
class bcel_focal(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        focal_factior = (1 - p_t) ** self.gamma
        loss = alpha_t * focal_factior * bce
...
class gaussian_bcel_focal(nn.Module):
    def __init__(self):
        super(gaussian_bcel_focal, self).__init__()
...
class Trainer:
    def __init__(self, config_dict, args):
        ...
        self.dataset = CRUW(data_root=self.config_dict['dataset_cfg']['base_root'], sensor_config_name=self.args.sensor_config, object_config_name=os.path.join(self.code_dir, 'configs/UAVRadar/dataset_configs/object_config'))
        ...
        self.crdata_train = CRDataset(data_dir=args.data_dir, dataset=self.dataset, config_dict=self.config_dict, split='train', noise_channel=self.args.use_noise_channel, is_random_chirp=True, use_training=True)
        self.seq_names = self.crdata_train.seq_names
        self.index_mapping = self.crdata_train.index_mapping
        self.train_dataloader = DataLoader(self.crdata_train, self.batch_size, shuffle=True, num_workers=self.train_configs['num_workers'], persistent_workers=True, pin_memory=True, prefetch_factor=2)
        ...
        for subset_idx in self.eval_seq_names:
            ...
            eval_dataloader = DataLoader(crdata_test, batch_size=1, shuffle=False, num_workers=self.train_configs['num_workers'], persistent_workers=True, collate_fn=cr_collate)
        ...
        elif self.model_cfg['type'] == 'STCT-Net':
        ...
        if self.model_cfg['type'] == 'CDC':
            ...
            self.model = Model(n_class_train).cuda()
        elif 'STCT-Net' == self.model_cfg['type']:
            self.model = Model(self.config_dict['model_cfg']['mnet_cfg'], n_class_train).cuda()
        else:
            raise TypeError
        ...
        elif self.train_configs['loss_type'] == 'bcel':
            self.criterion = nn.BCEWithLogitsLoss(reduction='mean')
        elif self.train_configs['loss_type'] == 'mse':
            self.criterion = nn.MSELoss(reduction='mean')
        elif self.train_configs['loss_type'] == 'smooth_l1':
            self.criterion = nn.SmoothL1Loss(reduction='mean')
        elif self.train_configs['loss_type'] == 'mae':
            self.criterion = nn.L1Loss(reduction='mean')
        elif self.train_configs['loss_type'] == 'bcel_focal':
            self.criterion = bcel_focal(reduction='mean')
        elif self.train_configs['loss_type'] == 'gaussian_bcel_focal':
            self.criterion = gaussian_bcel_focal()
        elif self.train_configs['loss_type'] == 'bcel_soft':
            self.criterion = bcel_soft(soft_beta=self.train_configs['soft_beta'], reduction='mean')
        elif self.train_configs['loss_type'] == 'smooth_l1l':
    ...
    def train(self, ):
        for epoch in range(self.n_epoch):
            ...
            for iter, data_dict in enumerate(self.train_dataloader):
                ...
                visual_data = data_dict[0]
                confmap_gt = data_dict[1].to(device="cuda", dtype=torch.float32, non_blocking=True)
                visual_gt = data_dict[1]
                ...
                if iter % self.config_dict['train_cfg']['log_step'] == 0:
                    ...
                    print('epoch %2d, iter %4d: loss: %.4f | load time: %.4f | back time: %.4f | lr: %.8f | best AP50 acc: %.4f | best AP70 acc: %.4f | best mAP acc: %.4f | best mAR acc: %.4f | best Score: %.4f | best Score epoch: %2d | wait time: %2d : %2d : %2d' % (epoch + 1, iter + 1, loss_confmap.item(), load_time, back_time, self.optimizer.param_groups[0]['lr'], self.best_ap50, self.best_ap70, self.best_map, self.best_mar, self.best_score, self.best_score_epoch, wait_h, wait_m, wait_s))
                    with open(self.train_log_name, 'a+') as f_log:
                        f_log.write('epoch %2d, iter %4d: loss: %.4f | load time: %.4f | back time: %.4f | lr: %.8f | best AP50 acc: %.4f | best AP70 acc: %.4f | best mAP acc: %.4f | best mAR acc: %.4f | best Score: %.4f | best Score epoch: %2d \n' % (epoch + 1, iter + 1, loss_confmap.item(), load_time, back_time, self.optimizer.param_groups[0]['lr'], self.best_ap50, self.best_ap70, self.best_map, self.best_mar, self.best_score, self.best_score_epoch))
                    ...
                    if 'bcel' in self.train_configs['loss_type'] or 'smooth_l1l' in self.train_configs['loss_type']:
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            dataset = CRUW(data_root=args.data_dir, sensor_config_name=args.sensor_config, object_config_name=os.path.join(self.code_dir, 'configs/UAVRadar/dataset_configs/object_config'))
            ...
            for seq_name in seq_names:
                ...
                evalImgs = evaluate_rodnet_seq_UAVRadar(res_path, gt_path, n_frame, dataset)
            ...
            print("%s | mAP50:90: %.4f | AP50: %.4f | AP70: %.4f | mAR50:90 %.4f" % ('Overall'.ljust(18), stats[0] * 100, stats[1] * 100, stats[3] * 100, stats[6] * 100))
            with open(self.train_log_name, 'a+') as f_log:
                f_log.write("%s | mAP50:90: %.4f | AP50: %.4f | AP70: %.4f | mAR50:90 %.4f \n" % ('Overall'.ljust(18), stats[0] * 100, stats[1] * 100, stats[3] * 100, stats[6] * 100))
            if not save_result:
                shutil.rmtree(test_res_dir)
        self.model.train()
        return stats[0], stats[1], stats[3], stats[6]
