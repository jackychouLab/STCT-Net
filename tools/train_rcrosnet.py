...
from rodnet.datasets.CRDataset_RCROSNet import CRDataset
...
class Trainer:
    def __init__(self, config_dict, args):
        ...
        if self.model_cfg['type'] == 'RC-ROSNet':
            from rodnet.models import RCRODNet as Model
        else:
            raise NotImplementedError

        print("Building model ... (%s)" % self.model_cfg)
        if self.model_cfg['type'] == 'RC-ROSNet':
            self.model = Model(n_classes=1, n_frames=32, depth=2, channels=64, deform_k=[3, 3, 3, 3, 3, 3, 3, 3]).cuda()
        else:
            raise TypeError
    ...
    def train(self, ):
        for epoch in range(self.n_epoch):
            ...
            for iter, data_dict in enumerate(self.train_dataloader):
                ...
                img_data = data_dict[1]
                img_data = img_data.to(device="cuda", dtype=torch.float32, non_blocking=True)
                confmap_gt = data_dict[2].to(device="cuda", dtype=torch.float32, non_blocking=True)
                visual_gt = data_dict[2]
                image_paths = data_dict[3]
                tic = time.perf_counter()
                self.optimizer.zero_grad()
                confmap_preds = self.model(img_data, data)
                loss_confmap = 0
        ...
        with torch.no_grad():
            ...
            for subset in tqdm(seq_names):
                ...
                for iter, data_dict in enumerate(eval_dataloader):
                    ...
                    confmap_pred = self.model(img_data.float().cuda(), data.float().cuda())
