...
class RadarObject():
    ...
    def generateHeatmap_all(self, frame):
        data_first = frame[:, :, 0::3, :]
        data_second = frame[:, :, 1::3, :]
        data_third = frame[:, :, 2::3, :]
        # F-Azimuth-Doppler-Range
        data_merge = np.concatenate([data_first, data_second, data_third], axis=1)
        if self.use_filter == 2 or self.use_filter == 4 or self.use_filter == 22 or self.use_filter == 44:
            for frame_idx in range(data_merge.shape[0]):
                for temp_idx in range(data_merge.shape[1]):
                    for temp_jdx in range(data_merge.shape[2]):
                        data_merge[frame_idx, temp_idx, temp_jdx, :] = butter_lowpass_filter(data_merge[frame_idx, temp_idx, temp_jdx, :], 6)

        if self.use_filter == 1 or self.use_filter == 4 or self.use_filter == 11 or self.use_filter == 44:
            data_merge = data_merge - np.mean(data_merge, axis=2, keepdims=True)

        # Range-FFT
        range_win = np.hamming(self.numADCSamples)
        merge_range = np.fft.fft(data_merge * range_win[np.newaxis, np.newaxis, np.newaxis, :], axis=3, n=self.rangeBins)
        ...
        # create RD
        # merge_RD = np.fft.fft(merge_range, axis=2, n=self.dopplerBins)
        # merge_RD = np.fft.fftshift(merge_RD, axes=2)
        # RD = np.mean(merge_RD[..., self.crop_num:-self.crop_num], axis=1, keepdims=False)
        # RD = np.transpose(RD, axes=[0, 2, 1])
        ...
        if self.use_filter == 11 or self.use_filter == 22 or self.use_filter == 33 or self.use_filter == 44 or self.use_filter == 10:
            ...
            RA = np.fft.fft(np.concatenate([merge_range[:, 0:4, ...], merge_range[:, -4:, ...]], axis=1) * azimuth_win[np.newaxis, :, np.newaxis, np.newaxis], axis=1, n=self.azimuthBins)
        else:
            RA = np.fft.fft(merge_range * azimuth_win[np.newaxis, :, np.newaxis, np.newaxis], axis=1, n=self.azimuthBins)
        RA = np.fft.fftshift(RA, axes=1)
        RA = RA[..., self.crop_num:-self.crop_num]
        RA = np.transpose(RA, axes=[0, 2, 3, 1])
        RA = np.concatenate([np.expand_dims(np.real(RA), axis=-1), np.expand_dims(np.imag(RA), axis=-1)], axis=-1)

        # create AD
        # if self.use_filter == 11 or self.use_filter == 22 or self.use_filter == 33 or self.use_filter == 44 or self.use_filter == 10:
        #     azimuth_win = taylor(self.numRX * 2)
        #     AD = np.fft.fft(np.concatenate([data_first, data_third], axis=1) * azimuth_win[np.newaxis, :, np.newaxis, np.newaxis], axis=1, n=self.azimuthBins)
        # else:
        #     AD = np.fft.fft(merge_RD * azimuth_win[np.newaxis, :, np.newaxis, np.newaxis], axis=1, n=self.azimuthBins)
        # AD = np.fft.fftshift(AD, axes=1)
        # AD = np.mean(AD[..., self.crop_num:-self.crop_num], axis=3, keepdims=False)
        # AD = np.concatenate([np.expand_dims(np.real(AD), axis=-1), np.expand_dims(np.imag(AD), axis=-1)], axis=-1)

        return RA


    def saveRadarData_all(self, save_data, save_path, start_idx, len):
        ...
        # ADs = ADs.astype(np.float32)
        for RA_fdx in range(len):
            RA = RAs[RA_fdx, ...]
            # RD = RDs[RA_fdx, ...]
            # AD = ADs[RA_fdx, ...]
            # np.save(os.path.join(AD_save_path, f"{RA_fdx+start_idx:09d}.npy"), AD)
            # np.save(os.path.join(RD_save_path, f"{RA_fdx+start_idx:09d}.npy"), RD)
            for RA_idx in range(RA.shape[0]):
                if RA_idx + 1 in self.chirp_list:
                    RA_item = RA[RA_idx, ...]
                    np.save(os.path.join(RA_save_path, f"{RA_fdx+start_idx:03d}_{RA_idx+1:09d}.npy"), RA_item)
    ...
    def processRadarDataAzimuth(self):
        for idxName in tqdm(range(len(self.radarDataFileNameGroup))):
            if 'Azimuth' in self.create_type:
                ...
                adcDataAzimuths = np.transpose(adcDataAzimuths, axes=[1, 0, 2, 3])
                for i in tqdm(range(0, len(adcDataAzimuths), 20)):
                    outputAzimuths = self.generateHeatmap_all(adcDataAzimuths[i:i+20, ...])
                    self.saveRadarData_all(outputAzimuths, os.path.join(self.saveDirNameGroup[idxName], 'azimuth'), i, 20)
                    del outputAzimuths
