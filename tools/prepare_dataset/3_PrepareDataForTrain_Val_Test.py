...
def load_anno_csv(csv_path, n_frame, dataset):
    if sensor_type == 'pcav1':
        folder_name_dict = dict(
            cam_0='camera_to_frame',
            rad_h=f'{file_type}_slice_frame_{use_filter}_PCAv1'
        )
    elif sensor_type == 'pcav2':
        folder_name_dict = dict(
            cam_0='camera_to_frame',
            rad_h=f'{file_type}_slice_frame_{use_filter}_PCAv2'
        )
    else:
        folder_name_dict = dict(
            cam_0='camera_to_frame',
    ...
    for seq in sets_seqs:
        ...
        try:
            ...
            if os.path.exists(image_dir):
                ...
                n_frame = None
            if sensor_type == 'pcav1':
                radar_dir = os.path.join(seq_path, f"{file_type}_slice_frame_{use_filter}_PCAv1/{radar_type}", dataset.sensor_cfg.radar_cfg['chirp_folder'])
            elif sensor_type == 'pcav2':
                radar_dir = os.path.join(seq_path, f"{file_type}_slice_frame_{use_filter}_PCAv2/{radar_type}", dataset.sensor_cfg.radar_cfg['chirp_folder'])
            else:
...
if __name__ == "__main__":
    # 修改 Start
    chirp_nums = [32, 64]  # 4 8 16
    use_filters = [11]  # 0 1 2 3 4
    rangeDownSamples = [4]  # 1 2 4
    radar_type = "azimuth"  # azimuth elevation
    data_root_path = "/home/jackychou/dataset/UAVRadar"
    label_type = "rad"
    use_split = True
    sensor_type = "pcav1" # pcav1 uniform pcav2
    # 修改 End
    range_length = 512
    for chirp_num in chirp_nums:
        for use_filter in use_filters:
            for rangeDownSample in rangeDownSamples:
                ...
                file_type = "python"
                if 'pca' in sensor_type:
                    sensor_config_path = f"/home/jackychou/Zhou/code/UAVRadar/cruw-devkit/cruw/dataset_configs/pca_{chirp_num}.json"
                else:
                    sensor_config_path = f"/home/jackychou/Zhou/code/UAVRadar/cruw-devkit/cruw/dataset_configs/{sensor_type}_{chirp_num}.json"
                SPLITS_LIST = ['train', 'valid', 'test', 'demo']
                config_path = f"/home/jackychou/Zhou/code/UAVRadar/configs/{config_name}"
                split_type = "train,test"
                if sensor_type == "pcav1" or sensor_type == "PCAv1":
                    out_data_dir_path = os.path.join(data_root_path, f"train_test_{chirp_num}_{use_filter}_{range_length // rangeDownSample}_PCAv1")
                elif sensor_type == "pcav2" or sensor_type == "PCAv2":
                    out_data_dir_path = os.path.join(data_root_path, f"train_test_{chirp_num}_{use_filter}_{range_length // rangeDownSample}_PCAv2")
                elif sensor_type == "uniform":
