...
class Trainer:
    def __init__(self, config_dict, args):
        ...
        elif self.model_cfg['type'] == 'refine':
            from rodnet.models import RODNetCDC_refine as Model
        else:
        ...
        if self.model_cfg['type'] == 'CDC':
            ...
            in_chirps = len(self.radar_configs['chirp_ids'])
            self.model = Model(in_channels=in_chirps, n_class=n_class_train, mnet_type=self.config_dict['model_cfg']['mnet_type'], backbone_type=self.config_dict['model_cfg']['backbone_type'], mnet_cfg=self.config_dict['model_cfg']['mnet_cfg'], train_type=self.config_dict['model_cfg']['train_type'], use_branch=self.config_dict['model_cfg']['use_branch'], branch_cfg=self.config_dict['model_cfg']['branch_cfg'], stft_cfg=self.config_dict['model_cfg']['stft_cfg']).cuda()
        else:
            raise TypeError
        ...
        if self.train_configs['loss_type'] == 'bcel':
        ...
        if self.train_configs['loss_type'] == 'bcel_focal':
            self.criterion = bcel_focal(reduction='mean')

        if self.train_configs['loss_type'] == 'gaussian_bcel_focal':
            self.criterion = gaussian_bcel_focal()

        if self.train_configs['loss_type'] == 'bcel_soft':
        ...
        for epoch in range(self.n_epoch):
            ...
            for iter, data_dict in enumerate(self.train_dataloader):
                ...
                if iter % self.config_dict['train_cfg']['log_step'] == 0:
                    ...
                    if 'bcel' in self.train_configs['loss_type']:
