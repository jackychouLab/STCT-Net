...
from rodnet.datasets.CRDataset_TransRadar import CRDataset
...
class Trainer:
    def __init__(self, config_dict, args):
        ...
        self.train_dataloader = DataLoader(self.crdata_train, self.batch_size, shuffle=True, num_workers=self.train_configs['num_workers'], persistent_workers=True)
        ...
        for subset_idx in self.eval_seq_names:
            ...
            eval_dataloader = DataLoader(crdata_test, batch_size=1, shuffle=False, num_workers=self.train_configs['num_workers'], collate_fn=cr_collate, multiprocessing_context='spawn', persistent_workers=True)
        ...
        if self.model_cfg['type'] == 'TransRadar':
            from rodnet.models import TransRad as Model
        else:
            raise NotImplementedError

        print("Building model ... (%s)" % self.model_cfg)
        if self.model_cfg['type'] == 'TransRadar':
            self.model = Model(n_classes=1, n_chirps=32, depth=8, channels=64, deform_k=[3, 3, 3, 3, 3, 3, 3, 3]).cuda()
        else:
            raise TypeError
    ...
    def train(self, ):
        for epoch in range(self.n_epoch):
            ...
            for iter, data_dict in enumerate(self.train_dataloader):
                ...
                data_ra = data_ra.to(device="cuda", dtype=torch.float32, non_blocking=True)
                data_rv = data_rv.to(device="cuda", dtype=torch.float32, non_blocking=True)
                data_va = data_va.to(device="cuda", dtype=torch.float32, non_blocking=True)
                confmap_gt = confmap_gt.to(device="cuda", dtype=torch.float32, non_blocking=True)
                confmap_preds = self.model(x_rd=data_rv, x_ra=data_ra, x_ad=data_va)
                loss_confmap = 0
                ...
                if iter % self.config_dict['train_cfg']['log_step'] == 0:
                    ...
                    print(
                        'epoch %2d, iter %4d: loss: %.4f | load time: %.4f | back time: %.4f | lr: %.8f | best AP50 acc: %.4f | best AP70 acc: %.4f | best mAP acc: %.4f | best mAR acc: %.4f | best Score: %.4f | best Score epoch: %2d | wait time: %2d : %2d : %2d' % (
                            epoch + 1, iter + 1, loss_confmap.item(), load_time, back_time,
                            self.optimizer.param_groups[0]['lr'], self.best_ap50, self.best_ap70, self.best_map,
                            self.best_mar, self.best_score, self.best_score_epoch, wait_h, wait_m, wait_s))
                    with open(self.train_log_name, 'a+') as f_log:
                        f_log.write(
                            'epoch %2d, iter %4d: loss: %.4f | load time: %.4f | back time: %.4f | lr: %.8f | best AP50 acc: %.4f | best AP70 acc: %.4f | best mAP acc: %.4f | best mAR acc: %.4f | best Score: %.4f | best Score epoch: %2d \n' % (
                                epoch + 1, iter + 1, loss_confmap.item(), load_time, back_time,
                                self.optimizer.param_groups[0]['lr'], self.best_ap50, self.best_ap70, self.best_map,
                                self.best_mar, self.best_score, self.best_score_epoch))
                    ...
                    fig_name = os.path.join(self.train_viz_path,
                                            '%03d_%010d_%06d.png' % (epoch + 1, self.iter_count, iter + 1))
                    img_path = image_paths[0][0]
                    visualize_train_img(fig_name, img_path, chirp_amp_curr, confmap_pred[0, :self.n_class, 0, :, :],
                                        visual_gt[0, :self.n_class, 0, :, :])
                self.iter_count += 1
        ...
        with torch.no_grad():
            ...
            for subset in tqdm(seq_names):
                ...
                for iter, data_dict in enumerate(eval_dataloader):
                    ...
                    data_rv = data_rv.to(device="cuda", dtype=torch.float32, non_blocking=True)
                    data_va = data_va.to(device="cuda", dtype=torch.float32, non_blocking=True)
                    confmap_pred = self.model(x_rd=data_rv, x_ra=data_ra, x_ad=data_va)
